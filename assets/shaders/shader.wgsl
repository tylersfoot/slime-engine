...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    if (normal.y > 0.5) {
        brightness = 1.0; // top +y
    } else if (normal.y < -0.5) {
        brightness = 0.4; // bottom -y
    } else if (abs(normal.z) > 0.5) {
        brightness = 0.8; // front/back +-z
    } else {
        brightness = 0.6; // left/right +-x
    }

    var result: vec3<f32> = object_color.rgb * brightness;

    let border_thickness = 0.01; // % of face width
    let u = fract(in.tex_coords.x);
    let v = fract(in.tex_coords.y);

    if (u < border_thickness || u > 1.0 - border_thickness || 
        v < border_thickness || v > 1.0 - border_thickness) {
